...
function fnValidateDeltaLogin(){
    ...
    if(objApiKey.value === ""){
        ...
        fetch("/liveStrategy1fo/validateLogin", requestOptions)
        ...
        .then(objResult => {
            if(objResult.status === "success"){
                ...
                // localStorage.setItem("NetLimitDSSLIVE1", JSON.stringify(objBalances));
                // console.log(localStorage.getItem("NetLimitDSSLIVE1"));

                $('#mdlDeltaLogin').modal('hide');
                localStorage.setItem("lsLoginValidDSSLIVE1", "true");
...
function fnToggleAutoTrader(){
    ...
    if(bAppStatus){
        if(isLsAutoTrader === null || isLsAutoTrader === "false"){
            ...
            localStorage.setItem("isAutoTraderDSSLIVE1", "true");
        }
        else{
