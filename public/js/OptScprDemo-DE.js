const gOPSDEStoragePrefix = "OPSDE__";
(function(){
    const objLs = window.localStorage;
    if(!objLs || objLs.__opsdePatched){
        return;
    }
    const fnMapKey = (pKey) => {
        const vKey = String(pKey || "");
        if(
            vKey.startsWith("DFSD_") ||
            vKey.startsWith("lsDFSD") ||
            vKey === "isDFSDAutoTrader" ||
            vKey === "lsTgBotTokenDFSD" ||
            vKey === "lsTgChatIdDFSD"
        ){
            return `${gOPSDEStoragePrefix}${vKey}`;
        }
        return vKey;
    };
    const fnBind = (pFnName) => objLs[pFnName].bind(objLs);
    const fnGetItem = fnBind("getItem");
    const fnSetItem = fnBind("setItem");
    const fnRemoveItem = fnBind("removeItem");
    objLs.getItem = (pKey) => fnGetItem(fnMapKey(pKey));
    objLs.setItem = (pKey, pVal) => fnSetItem(fnMapKey(pKey), pVal);
    objLs.removeItem = (pKey) => fnRemoveItem(fnMapKey(pKey));
    objLs.__opsdePatched = true;
})();
...
let gOptQuoteBusyOPSDE = false;
let gLastOptQuoteDtOPSDE = 0;
...
function fnOpsdeGetUnderlyingAsset(){
    const vSymbol = String(document.getElementById("ddlFuturesSymbols")?.value || "BTCUSD").toUpperCase();
    return vSymbol.startsWith("ETH") ? "ETH" : "BTC";
}

function fnOpsdeGetMappedTrade(pRenkoSide){
    const vSignal = String(pRenkoSide || "").toLowerCase();
    const vAction = String(document.getElementById("ddlAction1")?.value || "buy").toLowerCase();
    const vIsBuySignal = (vSignal === "buy");
    return {
        renkoSignal: vSignal,
        transType: (vAction === "sell") ? "sell" : "buy",
        optionType: (vAction === "buy")
            ? (vIsBuySignal ? "C" : "P")
            : (vIsBuySignal ? "P" : "C"),
        optionShort: (vAction === "buy")
            ? (vIsBuySignal ? "CE" : "PE")
            : (vIsBuySignal ? "PE" : "CE"),
        action: vAction
    };
...
function fnSyncOptQty(pVal){
    const objLegacyQty = document.getElementById("txtFuturesQty");
    const objOptQty = document.getElementById("txtStartQty1");
    let vQty = Math.floor(Number(pVal));
    if(!Number.isFinite(vQty) || vQty < 1){
        vQty = 1;
    }
    if(objLegacyQty){
        objLegacyQty.value = vQty;
    }
    if(objOptQty && Number(objOptQty.value) !== vQty){
        objOptQty.value = vQty;
    }
    localStorage.setItem("OPSDE_StartQty1", String(vQty));
}

function fnOpsdePersistConfig(){
    const objIds = [
        "ddlAction1",
        "ddlExpiryMode1",
        "txtExpiry1",
    ...
    objIds.forEach((pId) => {
        const objEl = document.getElementById(pId);
        if(objEl){
            localStorage.setItem(`OPSDE_${pId}`, String(objEl.value || ""));
        }
    });
}

function fnOpsdeLoadConfig(){
    [
        "ddlAction1",
        "ddlExpiryMode1",
        "txtExpiry1",
        ...
        const objEl = document.getElementById(pId);
        const vStored = localStorage.getItem(`OPSDE_${pId}`);
        if(objEl && vStored !== null && vStored !== ""){
            objEl.value = vStored;
        }
    });
    fnSyncOptQty(document.getElementById("txtStartQty1")?.value || 1);
}

function fnOpsdeComputeExpiry(pMode){
    const vMode = String(pMode || "4");
    ...
    const fnFmt = (pDate) => {
        ...
        const vMon = String(pDate.getMonth() + 1).padStart(2, "0");
        return `${pDate.getFullYear()}-${vMon}-${vDay}`;
    };

    if(vMode === "1"){
        vCurrDate.setDate(vCurrDate.getDate() + 1);
        return fnFmt(vCurrDate);
    }
    if(vMode === "2"){
    ...
    const vCurrDow = vCurrDate.getDay();
    const vDaysUntilFriday = (5 - vCurrDow + 7) % 7 || 7;
    vCurrFriday.setDate(vCurrDate.getDate() + vDaysUntilFriday);
    vNextFriday.setDate(vCurrFriday.getDate() + 7);

    if(vMode === "4"){
        return fnFmt(vCurrFriday);
    }
    if(vMode === "5"){
        return fnFmt(vNextFriday);
    }
    ...
    if(vMode === "6"){
        return fnFmt(vLastDayOfMonth);
    }
    if(vMode === "7"){
        return fnFmt(vLastDayOfNextMonth);
    }
    return fnFmt(vCurrFriday);
}

function fnOpsdeFormatExpiryForApi(pDateToConv){
    let vDate = new Date(pDateToConv);
    if(Number.isNaN(vDate.valueOf())){
        return "";
    }
    let vDay = String(vDate.getDate()).padStart(2, "0");
    let vMonth = String(vDate.getMonth() + 1).padStart(2, "0");
    let vYear = vDate.getFullYear();
    return `${vDay}-${vMonth}-${vYear}`;
}

function fnUpdateSellExpiryMode(){
    const objExpiryMode = document.getElementById("ddlExpiryMode1");
    const objExpiry = document.getElementById("txtExpiry1");
    if(!objExpiryMode || !objExpiry){
        return;
    }
    objExpiry.value = fnOpsdeComputeExpiry(objExpiryMode.value);
    fnOpsdePersistConfig();
}

function fnUpdateOptActionMapLabel(){
    const objMap = document.getElementById("txtAutoLegMap");
    if(!objMap){
        return;
    }
    const vAction = String(document.getElementById("ddlAction1")?.value || "buy").toLowerCase();
    objMap.value = (vAction === "sell") ? "PE on Buy Signal" : "CE on Buy Signal";
    fnOpsdePersistConfig();
}
...
function fnSetSymbolData(pThisVal){
    let objLotSize = document.getElementById("txtLotSize");
    const vVal = String(pThisVal || "BTCUSD").toUpperCase();
    localStorage.setItem("DFSD_DefSymbolM", JSON.stringify(vVal));
    if(objLotSize){
        objLotSize.value = vVal === "ETHUSD" ? "0.01" : "0.001";
    }
    gRenkoFeedAnchor = null;
    gRenkoFeedLastDir = 0;
    fnLoadPrevRenkoCloseSettings();
...
function fnOpsdeGetOptionTradeCharges(pIndexPrice, pLotSize, pQty, pBuyPrice, pSellPrice, pOptionType){
    const vIdx = Number(pIndexPrice);
    const vLot = Number(pLotSize);
    const vQty = Number(pQty);
    const vBuy = Number(pBuyPrice);
    const vSell = Number(pSellPrice);
    if(!Number.isFinite(vIdx) || !Number.isFinite(vLot) || !Number.isFinite(vQty) || !Number.isFinite(vBuy) || !Number.isFinite(vSell)){
        return 0;
    }
    let vNotionalFees = (((vQty * 2) * vLot * vIdx) * gOptionBrokerage) / 100;
    let vBuyBrokerage = ((vQty * vLot * vBuy) * 3.5) / 100;
    let vSellBrokerage = ((vQty * vLot * vSell) * 3.5) / 100;
    let vPremiumCapFees = vSellBrokerage + vBuyBrokerage;
    return Math.min(vPremiumCapFees, vNotionalFees) * 1.18;
}

function fnOpsdeGetOptionTradePL(pBuyPrice, pSellPrice, pLotSize, pQty, pCharges){
    return ((Number(pSellPrice) - Number(pBuyPrice)) * Number(pLotSize) * Number(pQty)) - Number(pCharges || 0);
}

function fnOpsdeGetCloseRates(pTrade, pBestRates){
    const vBid = Number(pBestRates?.result?.quotes?.best_bid);
    const vAsk = Number(pBestRates?.result?.quotes?.best_ask);
    let vBuyPrice = Number(pTrade?.BuyPrice);
    let vSellPrice = Number(pTrade?.SellPrice);
    if(String(pTrade?.TransType || "").toLowerCase() === "buy"){
        if(Number.isFinite(vBid)){
            vSellPrice = vBid;
        }
    }
    else{
        if(Number.isFinite(vAsk)){
            vBuyPrice = vAsk;
        }
    }
    return { buyPrice: vBuyPrice, sellPrice: vSellPrice, bid: vBid, ask: vAsk };
}

function fnOpsdeGetOptionBestRatesBySymbol(pSymbol){
    ...
    return new Promise((resolve) => {
        ...
        fetch("/strategyfogreeks/getBestRatesBySymb", {
            method: "POST",
            headers: vHeaders,
            body: vAction,
            redirect: "follow"
        ...
        .catch(() => resolve({ status: "danger", data: "" }));
    });
}
...
function fnExecOptionLeg(pApiKey, pSecret, pOrderType, pUndrAsst, pExpiry, pOptionType, pTransType, pLotSize, pLotQty, pDeltaPos, pDeltaRePos, pDeltaTP, pDeltaSL){
    return new Promise((resolve) => {
        ...
        fetch("/strategyfogreeks/execOptionLeg", {
            method: "POST",
            headers: vHeaders,
            body: vAction,
            redirect: "follow"
        ...
        .then(objResult => {
            ...
            if(objResult.status === "danger" && objResult.data?.response?.body?.error?.code){
                const vErr = objResult.data.response.body.error.code;
                resolve({ status: objResult.status, message: `${vErr} Contact Admin!`, data: objResult.data });
                return;
            }
            resolve({ status: objResult.status || "warning", message: objResult.message || "Option execution failed.", data: objResult.data || "" });
        })
        .catch(() => resolve({ status: "danger", message: "Error while executing option leg.", data: "" }));
    });
}

async function fnOpsdeRefreshOpenOptionQuote(pForce = false){
    if(gOptQuoteBusyOPSDE){
        return;
    }
    if(!gCurrPos || !Array.isArray(gCurrPos.TradeData) || gCurrPos.TradeData.length === 0){
        return;
    ...
    const objTrade = gCurrPos.TradeData[0];
    const vNow = Date.now();
    if(!pForce && (vNow - gLastOptQuoteDtOPSDE) < 1200){
        return;
    }
    gOptQuoteBusyOPSDE = true;
    try{
        const objRates = await fnOpsdeGetOptionBestRatesBySymbol(objTrade.Symbol || objTrade.FutSymbol);
        if(objRates.status !== "success"){
            return;
        }
        gLastOptQuoteDtOPSDE = Date.now();
        const objCurr = fnOpsdeGetCloseRates(objTrade, objRates.data);
        const vCharges = fnOpsdeGetOptionTradeCharges(objTrade.StrikePrice || objTrade.IndexPrice || 0, objTrade.LotSize, objTrade.Qty, objCurr.buyPrice, objCurr.sellPrice, objTrade.OptionType);
        const vPL = fnOpsdeGetOptionTradePL(objCurr.buyPrice, objCurr.sellPrice, objTrade.LotSize, objTrade.Qty, vCharges);
        objTrade.BuyPrice = objCurr.buyPrice;
        objTrade.SellPrice = objCurr.sellPrice;
        objTrade.Charges = vCharges;
        objTrade.ProfitLoss = vPL;
        localStorage.setItem("DFSD_CurrFutPos", JSON.stringify(gCurrPos));
        gBuyPrice = objCurr.buyPrice;
        gSellPrice = objCurr.sellPrice;
        gCharges = vCharges;
        gPL = vPL;

        const objBuyPrice = document.getElementById("tdBuyPrice");
        const objSellPrice = document.getElementById("tdSellPrice");
        const objCapital = document.getElementById("tdCapital");
        const objCharges = document.getElementById("tdCharges");
        const objProfitLoss = document.getElementById("tdProfitLoss");

        if(objBuyPrice){ objBuyPrice.innerHTML = Number(objCurr.buyPrice).toFixed(2); }
        if(objSellPrice){ objSellPrice.innerHTML = Number(objCurr.sellPrice).toFixed(2); }
        if(objCapital){ objCapital.innerText = Math.abs(Number(objTrade.LotSize) * Number(objTrade.Qty) * Math.max(Number(objCurr.buyPrice), Number(objCurr.sellPrice), 0)).toFixed(2); }
        if(objCharges){ objCharges.innerText = Number(vCharges).toFixed(2); }
        if(objProfitLoss){ objProfitLoss.innerText = Number(vPL).toFixed(2); }
        fnUpdateCurrentSLDisplay();
        fnUpdateOpenPositions();
    }
    finally{
        gOptQuoteBusyOPSDE = false;
    }
}

function fnUpdateOpnPosStatus(){
    fnOpsdeRefreshOpenOptionQuote();
}

function fnOpsdeBuildOpenTrade(pExecData, pRenkoSignal, pMappedTrade){
    const vNow = new Date();
    const vOrderId = Number(pExecData?.TradeID) || vNow.valueOf();
    const vOpenDt = `${vNow.getDate()}-${vNow.getMonth() + 1}-${vNow.getFullYear()} ${vNow.getHours()}:${vNow.getMinutes()}:${vNow.getSeconds()}`;
    const vBuyPrice = Number(pExecData?.BuyPrice ?? pExecData?.BestAsk ?? 0);
    const vSellPrice = Number(pExecData?.SellPrice ?? pExecData?.BestBid ?? 0);
    ...
    const vTPAmount = fnParsePositiveNumber(objTPAmount?.value, 0);
    let vAmtSL = 0;
    let vAmtTP1 = 0;
    let vTrailNextTrigger = null;
    if(pMappedTrade.transType === "buy"){
        vAmtSL = Number.isFinite(vSLPoints) ? Number((vBuyPrice - vSLPoints).toFixed(2)) : 0;
        vAmtTP1 = Number.isFinite(vTPPoints1) ? Number((vBuyPrice + vTPPoints1).toFixed(2)) : 0;
        if(vTSLPoints > 0){
            vTrailNextTrigger = Number((vSellPrice + vTSLPoints).toFixed(2));
        }
    }
    else{
        vAmtSL = Number.isFinite(vSLPoints) ? Number((vSellPrice + vSLPoints).toFixed(2)) : 0;
        vAmtTP1 = Number.isFinite(vTPPoints1) ? Number((vSellPrice - vTPPoints1).toFixed(2)) : 0;
        if(vTSLPoints > 0){
            vTrailNextTrigger = Number((vBuyPrice - vTSLPoints).toFixed(2));
        }
    }
    ...
    return {
        ...
        OpenDT: vOpenDt,
        OpenDTVal: vOrderId,
        FutSymbol: String(pExecData?.Symbol || ""),
        Symbol: String(pExecData?.Symbol || ""),
        Underlying: String(pExecData?.UndrAsstSymb || fnOpsdeGetUnderlyingAsset()),
        Expiry: String(pExecData?.Expiry || document.getElementById("txtExpiry1")?.value || ""),
        StrikePrice: Number(pExecData?.StrikePrice ?? pExecData?.Strike ?? 0),
        OptionType: pMappedTrade.optionShort,
        SignalType: String(pRenkoSignal || "").toUpperCase(),
        ActionCfg: pMappedTrade.action,
        TransType: pMappedTrade.transType,
        LotSize: Number(pExecData?.LotSize || document.getElementById("txtLotSize")?.value || 0),
        Qty: Number(pExecData?.LotQty || document.getElementById("txtStartQty1")?.value || 1),
        BuyPrice: vBuyPrice,
        SellPrice: vSellPrice,
        Delta: Number(pExecData?.Delta ?? 0),
        DeltaNP: Number(pExecData?.DeltaNP ?? (document.getElementById("txtNewDelta1")?.value || 0)),
        DeltaRePos: Number(pExecData?.DeltaRePos ?? (document.getElementById("txtReDelta1")?.value || 0)),
        DeltaTP: Number(pExecData?.DeltaTP ?? (document.getElementById("txtDeltaTP1")?.value || 0)),
        DeltaSL: Number(pExecData?.DeltaSL ?? (document.getElementById("txtDeltaSL1")?.value || 0)),
        AmtSL: vAmtSL,
        AmtTP1: vAmtTP1,
        AmtTP: vTPAmount,
        ...
        IndexPrice: Number(document.getElementById("txtSpotPrice")?.value || pExecData?.StrikePrice || 0)
    };
}
...
async function fnInitiateManualFutures(pTransType){
    ...
    if(isLsAutoTrader !== "true"){
        fnGenMessage("Trade not executed, Auto Trader is OFF!", "badge bg-warning", "spnGenMsg");
        return;
    }
    if(objCurrPos !== null){
        fnGenMessage("Close the open option position before executing a new one!", "badge bg-warning", "spnGenMsg");
        return;
    }
    ...
    const objQty = document.getElementById("txtStartQty1");
    const objExpiry = document.getElementById("txtExpiry1");
    const objNewDelta = document.getElementById("txtNewDelta1");
    const objReDelta = document.getElementById("txtReDelta1");
    const objTP = document.getElementById("txtDeltaTP1");
    const objSL = document.getElementById("txtDeltaSL1");

    if(!objApiKey?.value || !objApiSecret?.value){
        fnGenMessage("Please login with API Key and Secret first.", "badge bg-warning", "spnGenMsg");
        return;
    }
    if(!objExpiry?.value){
    ...
    const objMappedTrade = fnOpsdeGetMappedTrade(pTransType);
    const vLotQty = Math.max(1, Math.floor(Number(objQty?.value || 1)));
    objQty.value = vLotQty;
    fnSyncOptQty(vLotQty);

    const vExpiryApi = fnOpsdeFormatExpiryForApi(objExpiry?.value || "");
    if(!vExpiryApi){
        fnGenMessage("Please select a valid expiry date.", "badge bg-warning", "spnGenMsg");
        return;
    }
    ...
    let objTradeDtls = await fnExecOptionLeg(
        objApiKey.value,
        objApiSecret.value,
        objOrderType?.value || "market_order",
        fnOpsdeGetUnderlyingAsset(),
        vExpiryApi,
        objMappedTrade.optionType,
        objMappedTrade.transType,
        objLotSize?.value || 0,
        vLotQty,
        objNewDelta?.value || 0,
        objReDelta?.value || 0,
    ...
    if(objTradeDtls.status !== "success"){
        fnGenMessage(objTradeDtls.message || "Option trade execution failed!", `badge bg-${objTradeDtls.status || "danger"}`, "spnGenMsg");
        return;
    }

    const objTrade = fnOpsdeBuildOpenTrade(objTradeDtls.data, pTransType, objMappedTrade);
    gCurrPos = { TradeData: [objTrade] };
    gByorSl = objTrade.TransType;
    gOrderDT = objTrade.OrderID;
    localStorage.setItem("DFSD_CurrFutPos", JSON.stringify(gCurrPos));
    localStorage.setItem("DFSD_QtyMul", String(vLotQty));
    g50Perc1Time = true;

    fnSetInitFutTrdDtls();
    fnSubscribe();
    fnGenMessage(`Option ${objTrade.OptionType} ${objTrade.TransType.toUpperCase()} executed from renko ${objTrade.SignalType}.`, "badge bg-success", "spnGenMsg");
    document.getElementById("spnLossTrd").className = "badge rounded-pill text-bg-success";
    fnSendTelegramRuntimeAlert(`OptScprDemo-DE\nOption Opened\nUnderlying: ${objTrade.Underlying}\nSignal: ${objTrade.SignalType}\nLeg: ${objTrade.OptionType}\nAction: ${objTrade.TransType}\nSymbol: ${objTrade.Symbol}`);
    fnOpsdeRefreshOpenOptionQuote(true);
}
...
function fnSetInitFutTrdDtls(){
    ...
    let objCharges = document.getElementById("tdCharges");
    let objProfitLoss = document.getElementById("tdProfitLoss");
    if(gCurrPos !== null && gCurrPos.TradeData && gCurrPos.TradeData[0]){
        const objTrade = gCurrPos.TradeData[0];
        gOrderDT = objTrade.OrderID;
        gBuyPrice = Number(objTrade.BuyPrice);
        gSellPrice = Number(objTrade.SellPrice);
        gLotSize = Number(objTrade.LotSize);
        gQty = Number(objTrade.Qty);
        gByorSl = objTrade.TransType;
        gAmtSL = objTrade.AmtSL;
        gAmtTP1 = objTrade.AmtTP1;
        gAmtTP = objTrade.TakeProfitAmt;

        if(objDateTime){ objDateTime.innerText = objTrade.OpenDT; }
        if(objAction){
            objAction.innerHTML = '<i class="fa fa-eye" aria-hidden="true" style="color:green; cursor:pointer;" title="Close This Trade" onclick="fnCloseOpenTradeAction();"></i>&nbsp;&nbsp;&nbsp;<i class="fa fa-wrench" aria-hidden="true" style="color:#01ff1f; cursor:pointer;" title="Edit This Trade" onclick="fnOpenEditCurrentTrade();"></i>&nbsp;&nbsp;&nbsp;<i class="fa fa-trash-o" aria-hidden="true" style="color:red; cursor:pointer;" title="Delete This Trade" onclick="fnDeleteOpenTradeAction();"></i>';
        }
        if(objSymbol){
            objSymbol.innerText = `${objTrade.Symbol} | ${objTrade.OptionType} | ${objTrade.SignalType}`;
        }
        if(objTransType){ objTransType.innerText = objTrade.TransType; }
        if(objLotSize){ objLotSize.innerText = objTrade.LotSize; }
        if(objQty){ objQty.innerText = objTrade.Qty; }
        if(objBuyPrice){ objBuyPrice.innerHTML = Number(objTrade.BuyPrice).toFixed(2); }
        if(objSellPrice){ objSellPrice.innerHTML = Number(objTrade.SellPrice).toFixed(2); }
        if(objCapital){ objCapital.innerText = Math.abs(Number(objTrade.LotSize) * Number(objTrade.Qty) * Math.max(Number(objTrade.BuyPrice), Number(objTrade.SellPrice))).toFixed(2); }
        if(objCharges){ objCharges.innerText = "0.00"; }
        if(objProfitLoss){ objProfitLoss.innerText = "0.00"; }

        fnSet50PrctQty();
        fnGenMessage("<span class='blink'>Option position is open</span>", "badge bg-warning", "btnPositionStatus");
        fnOpsdeRefreshOpenOptionQuote(true);
    }
    else{
        if(objAction){ objAction.innerText = ""; }
        if(objDateTime){ objDateTime.innerText = ""; }
        if(objSymbol){ objSymbol.innerText = ""; }
        if(objTransType){ objTransType.innerText = ""; }
        if(objLotSize){ objLotSize.innerText = ""; }
        if(objQty){ objQty.innerText = ""; }
        if(objBuyPrice){ objBuyPrice.innerText = ""; }
        if(objSellPrice){ objSellPrice.innerText = ""; }
        if(objCapital){ objCapital.innerText = ""; }
        if(objCharges){ objCharges.innerText = ""; }
        if(objProfitLoss){ objProfitLoss.innerText = ""; }
        gByorSl = "";
    }
    fnUpdateCurrentSLDisplay();
}
...
async function fnCloseManualFutures(pTransType){
    if(gCurrPos === null || !gCurrPos.TradeData?.[0]){
        fnGenMessage("No open option position to close!", "badge bg-warning", "spnGenMsg");
        return;
    }
    if(gCurrPos.TradeData[0].TransType !== pTransType){
        fnGenMessage(`No ${pTransType} option position to close!`, "badge bg-warning", "spnGenMsg");
        return;
    }
    let objClsTrd = await fnInnitiateClsFutTrade(0);
    fnGenMessage(objClsTrd.message, `badge bg-${objClsTrd.status}`, "spnGenMsg");
...
async function fnInnitiateClsFutTrade(pQty){
    if(!gCurrPos || !gCurrPos.TradeData?.[0]){
        return { status: "warning", message: "No open position found.", data: "" };
    }
    const objTrade = gCurrPos.TradeData[0];
    const vCurrQty = Number(objTrade.Qty);
    const vCloseQtyReq = Number(pQty);
    if(Number.isNaN(vCloseQtyReq) || vCloseQtyReq < 0 || vCloseQtyReq > vCurrQty){
        return { status: "warning", message: "Invalid quantity requested for close.", data: "" };
    }
    const vExecQty = (vCloseQtyReq === 0) ? vCurrQty : vCloseQtyReq;
    const vRemainingQty = vCurrQty - vExecQty;
    const objBestRates = await fnOpsdeGetOptionBestRatesBySymbol(objTrade.Symbol || objTrade.FutSymbol);
    if(objBestRates.status !== "success"){
        return { status: "warning", message: "Unable to fetch option close rates.", data: "" };
    }

    const vDate = new Date();
    const vToday = `${vDate.getDate()}-${vDate.getMonth() + 1}-${vDate.getFullYear()} ${vDate.getHours()}:${vDate.getMinutes()}:${vDate.getSeconds()}`;
    const objCloseRates = fnOpsdeGetCloseRates(objTrade, objBestRates.data);
    const objClosedTrade = {
        ...objTrade,
        Qty: vExecQty,
        BuyPrice: objCloseRates.buyPrice,
        SellPrice: objCloseRates.sellPrice,
        CloseDT: vToday,
        CloseDTVal: vDate.valueOf(),
        Status: "CLOSED"
    };
    objClosedTrade.Capital = Math.abs(Number(objClosedTrade.LotSize) * Number(objClosedTrade.Qty) * Math.max(Number(objClosedTrade.BuyPrice), Number(objClosedTrade.SellPrice)));
    objClosedTrade.Charges = fnOpsdeGetOptionTradeCharges(objClosedTrade.StrikePrice || objClosedTrade.IndexPrice || 0, objClosedTrade.LotSize, objClosedTrade.Qty, objClosedTrade.BuyPrice, objClosedTrade.SellPrice, objClosedTrade.OptionType);
    objClosedTrade.ProfitLoss = fnOpsdeGetOptionTradePL(objClosedTrade.BuyPrice, objClosedTrade.SellPrice, objClosedTrade.LotSize, objClosedTrade.Qty, objClosedTrade.Charges);

    gOldPLAmt = Number(localStorage.getItem("DFSD_TotLossAmt")) || 0;
    gNewPLAmt = Number(objClosedTrade.ProfitLoss) || 0;
    localStorage.setItem("DFSD_OldPLAmt", gOldPLAmt);
    localStorage.setItem("DFSD_NewPLAmt", gNewPLAmt);
    fnApplyLossCarryFromPLDemo(gNewPLAmt);

    const objTodayTrades = fnGetLsJSON("DFSD_TrdBkFut", { TradeData: [] }) || { TradeData: [] };
    if(!Array.isArray(objTodayTrades.TradeData)){
        objTodayTrades.TradeData = [];
    }
    objTodayTrades.TradeData.push(objClosedTrade);
    localStorage.setItem("DFSD_TrdBkFut", JSON.stringify(objTodayTrades));

    if(vRemainingQty > 0){
        gCurrPos.TradeData[0].Qty = vRemainingQty;
        localStorage.setItem("DFSD_CurrFutPos", JSON.stringify(gCurrPos));
    }
    else{
        localStorage.removeItem("DFSD_CurrFutPos");
        gCurrPos = null;
        fnGenMessage("No Open Position", "badge bg-success", "btnPositionStatus");
        fnSetNextOptTradeSettings(true);
    }

    document.getElementById("spnLossTrd").className = "badge rounded-pill text-bg-danger";
    fnSendTelegramRuntimeAlert(`OptScprDemo-DE\n${vRemainingQty > 0 ? "Partial Close" : "Position Closed"}\nSymbol: ${objTrade.Symbol}\nClosed Qty: ${vExecQty}\nRemaining Qty: ${Math.max(0, vRemainingQty)}`);
    clearInterval(gTimerID);
    return { status: "success", message: "Option paper trade closed successfully!", data: "" };
}
...
function fnLoadTodayTrades(){
    ...
    objTradeBook.TradeData.forEach((vTrade) => {
        const vCharges = Number(vTrade.Charges ?? (fnOpsdeGetOptionTradeCharges(vTrade.StrikePrice || vTrade.IndexPrice || 0, vTrade.LotSize, vTrade.Qty, vTrade.BuyPrice, vTrade.SellPrice, vTrade.OptionType)));
        const vCapital = Number(vTrade.Capital ?? Math.abs(Number(vTrade.LotSize) * Number(vTrade.Qty) * Math.max(Number(vTrade.BuyPrice), Number(vTrade.SellPrice))));
        const vPL = Number(vTrade.ProfitLoss ?? fnOpsdeGetOptionTradePL(vTrade.BuyPrice, vTrade.SellPrice, vTrade.LotSize, vTrade.Qty, vCharges));
        vTotalTrades += 1;
        ...
        [
            vTrade.OpenDT,
            vTrade.CloseDT,
            `${vTrade.Symbol} ${vTrade.OptionType || ""}`.trim(),
            `${vTrade.TransType}/${vTrade.SignalType || "-"}`,
            vTrade.LotSize,
            vTrade.Qty,
            Number(vTrade.BuyPrice).toFixed(2),
            Number(vTrade.SellPrice).toFixed(2),
            ...
            Number(vCharges).toFixed(2),
            Number(vPL).toFixed(2)
        ].forEach((vCellVal, vIdx) => {
            const td = document.createElement("td");
            td.style.textWrap = "nowrap";
            td.textContent = String(vCellVal);
            if(vIdx >= 4){ td.style.textAlign = "right"; }
            if(vIdx === 2){ td.style.fontWeight = "bold"; td.style.textAlign = "left"; }
            if(vIdx === 6){ td.style.color = "green"; }
            if(vIdx === 7){ td.style.color = "red"; }
            if(vIdx === 8 || vIdx === 9){ td.style.color = "orange"; }
            vRow.appendChild(td);
        });
        objTodayTradeList.appendChild(vRow);
    });
    ...
    const vTotalRow = document.createElement("tr");
    ["", "Total Trades", vTotalTrades, "", "", "", "", "", "", Number(vHighCapital).toFixed(2), Number(vTotalCharges).toFixed(2), Number(vNetProfit).toFixed(2)].forEach((vCellVal, vIdx) => {
        const td = document.createElement("td");
        td.textContent = String(vCellVal);
        td.style.textWrap = "nowrap";
        if(vIdx >= 9){ td.style.textAlign = "right"; td.style.fontWeight = "bold"; }
        if(vIdx === 10){ td.style.color = "red"; }
        vTotalRow.appendChild(td);
    });
    objTodayTradeList.appendChild(vTotalRow);
    fnSetTextByPL(objHeadPL, vNetProfit);
...
window.addEventListener("DOMContentLoaded", function(){
    fnOpsdeLoadConfig();
    const objAction = document.getElementById("ddlAction1");
    const objQty = document.getElementById("txtStartQty1");
    const objExpiryMode = document.getElementById("ddlExpiryMode1");
    const objExpiry = document.getElementById("txtExpiry1");
    [objAction, objQty, objExpiryMode, objExpiry].forEach((objEl) => {
        if(objEl){
            objEl.addEventListener("change", fnOpsdePersistConfig);
        }
    });
    if(objAction){
        objAction.addEventListener("change", fnUpdateOptActionMapLabel);
    }
    if(objQty){
        objQty.addEventListener("change", () => fnSyncOptQty(objQty.value));
    }
    fnUpdateOptActionMapLabel();
    if(!document.getElementById("txtExpiry1")?.value){
        fnUpdateSellExpiryMode();
    }
});

function fnOpsdeGetOpenTradeList(){
    if(!gCurrPos || !Array.isArray(gCurrPos.TradeData)){
        return [];
    }
    return gCurrPos.TradeData.filter(objTrade => objTrade);
}

function fnOpsdeGetClosedTradeList(){
    const objTradeBook = fnGetLsJSON("DFSD_TrdBkFut", { TradeData: [] });
    if(!objTradeBook || !Array.isArray(objTradeBook.TradeData)){
        return [];
    }
    return objTradeBook.TradeData.filter(objTrade => objTrade);
}

function fnOpsdeGreekCell(pCurrent, pBase, pDecimals = 2, pCurrentColor = "orange"){
    const vCurr = Number(pCurrent || 0);
    const vBase = Number(pBase || 0);
    return '<div style="text-wrap: nowrap; text-align:right; font-weight:bold; color:' + pCurrentColor + ';">' + vCurr.toFixed(pDecimals) + '</div>'
        + '<div style="text-wrap: nowrap; text-align:right; font-weight:bold; color:grey;">' + vBase.toFixed(pDecimals) + '</div>';
}

function fnOpsdeBuildTradeMetrics(objTrade){
    ...
    const vStrike = Number(objTrade?.StrikePrice || objTrade?.IndexPrice || 0);
    const vDelta = Number(objTrade?.Delta || 0);
    const vDeltaC = Number(objTrade?.DeltaC ?? objTrade?.Delta ?? 0);
    const vGamma = Number(objTrade?.Gamma || 0);
    const vGammaC = Number(objTrade?.GammaC ?? objTrade?.Gamma ?? 0);
    const vTheta = Number(objTrade?.Theta || 0);
    const vThetaC = Number(objTrade?.ThetaC ?? objTrade?.Theta ?? 0);
    const vVega = Number(objTrade?.Vega || 0);
    const vVegaC = Number(objTrade?.VegaC ?? objTrade?.Vega ?? 0);
    const vCharges = Number(objTrade?.Charges ?? fnOpsdeGetOptionTradeCharges(vStrike, vLotSize, vQty, vBuyPrice, vSellPrice, objTrade?.OptionType));
    const vPL = Number(objTrade?.ProfitLoss ?? fnOpsdeGetOptionTradePL(vBuyPrice, vSellPrice, vLotSize, vQty, vCharges));
    const vGreekFactor = Number.isFinite(vQty) && vQty > 0 ? vQty : 1;
...
function fnUpdateOpenPositions(){
    const objCurrTradeList = document.getElementById("tBodyCurrTrades");
    if(!objCurrTradeList){
        return;
    }

    const objTrades = fnOpsdeGetOpenTradeList();
    if(objTrades.length === 0){
        objCurrTradeList.innerHTML = '<tr><td colspan="16"><div class="col-sm-12" style="width:100%;text-align:center;font-weight:bold;font-size:40px;">No Running Trades Yet</div></td></tr>';
        gPL = 0;
        return;
    }
    ...
    let vNetPL = 0;
    let vTotalDelta = 0;
    let vTotalDeltaBase = 0;
    let vTotalGamma = 0;
    let vTotalGammaBase = 0;
    let vTotalTheta = 0;
    let vTotalThetaBase = 0;
    let vTotalVega = 0;
    let vTotalVegaBase = 0;

    objTrades.forEach((objTrade) => {
        const vLegID = Number(objTrade.TradeID || objTrade.OrderID || 0);
        const vSymbol = String(objTrade.Symbol || objTrade.FutSymbol || "-");
        const vTransType = String(objTrade.TransType || "-");
        const vOptionType = String(objTrade.OptionType || "");
        const vOpenDT = objTrade.OpenDT || "-";
        const vCloseDT = objTrade.CloseDT || "-";
        const vStatus = objTrade.Status || "OPEN";
        const objMet = fnOpsdeBuildTradeMetrics(objTrade);

        vTotalCharges += objMet.charges;
        vNetPL += objMet.pl;
        vTotalDelta += objMet.delta;
        vTotalDeltaBase += objMet.deltaBase;
        vTotalGamma += objMet.gamma;
        vTotalGammaBase += objMet.gammaBase;
        vTotalTheta += objMet.theta;
        vTotalThetaBase += objMet.thetaBase;
        vTotalVega += objMet.vega;
        vTotalVegaBase += objMet.vegaBase;

        vTempHtml += '<tr>';
        vTempHtml += '<td style="text-wrap: nowrap;"><i class="fa fa-eye" aria-hidden="true" style="color:green;" title="Close This Leg!" onclick="fnCloseManualFutures(`' + vTransType + '`);"></i>&nbsp;&nbsp;&nbsp;<i class="fa fa-wrench" aria-hidden="true" style="color:#01ff1f;" title="Edit This Trade" onclick="fnOpenEditModel(' + vLegID + ', ' + objMet.lotSize + ', ' + objMet.qty + ', `' + objMet.buyPrice + '`, `' + objMet.sellPrice + '`, `open`);"></i>&nbsp;&nbsp;&nbsp;<i class="fa fa-trash-o" aria-hidden="true" style="color:red;" title="Delete This Trade" onclick="fnDeleteOpenTradeAction();"></i></td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.delta, objMet.deltaBase, 2) + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.gamma, objMet.gammaBase, 4) + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.theta, objMet.thetaBase, 4) + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.vega, objMet.vegaBase, 4) + '</td>';
        ...
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right;">' + objMet.lotSize + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right;">' + objMet.qty + '</td>';
        if(vTransType === "sell"){
            vTempHtml += '<td style="text-wrap: nowrap; color:white;text-align:right;"><span class="blink">' + objMet.buyPrice.toFixed(2) + '</span></td>';
            vTempHtml += '<td style="text-wrap: nowrap; color:red;text-align:right;">' + objMet.sellPrice.toFixed(2) + '</td>';
        }
        else{
            vTempHtml += '<td style="text-wrap: nowrap; color:red;text-align:right;">' + objMet.buyPrice.toFixed(2) + '</td>';
            vTempHtml += '<td style="text-wrap: nowrap; color:white;text-align:right;"><span class="blink">' + objMet.sellPrice.toFixed(2) + '</span></td>';
        }
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right;">' + objMet.charges.toFixed(2) + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right;color:#ff9a00;">' + objMet.pl.toFixed(2) + '</td>';
    ...
    vTempHtml += '<tr>';
    vTempHtml += '<td></td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalDelta, vTotalDeltaBase, 2) + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalGamma, vTotalGammaBase, 4) + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalTheta, vTotalThetaBase, 4) + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalVega, vTotalVegaBase, 4) + '</td>';
...
function fnDispClosedPositions(){
    ...
    const objTrades = fnOpsdeGetClosedTradeList();
    if(objTrades.length === 0){
        objClosedTradeList.innerHTML = '<tr><td colspan="16"><div class="col-sm-12" style="width:100%;text-align:center;font-weight:bold;font-size:32px;">No Closed Trades Yet</div></td></tr>';
        return;
    }
    ...
    objTrades.forEach((objTrade) => {
        ...
        const vOpenDT = objTrade.OpenDT || "-";
        const vCloseDT = objTrade.CloseDT || "-";
        const vStatus = objTrade.Status || "CLOSED";
        const objMet = fnOpsdeBuildTradeMetrics(objTrade);
        ...
        vTempHtml += '<tr>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:center;"><i class="fa fa-wrench" aria-hidden="true" style="color:#01ff1f;" title="Edit Trade" onclick="fnOpenEditModel(' + vLegID + ', ' + objMet.lotSize + ', ' + objMet.qty + ', `' + objMet.buyPrice + '`, `' + objMet.sellPrice + '`, `closed`);"></i>&nbsp;&nbsp;&nbsp;<i class="fa fa-trash-o" aria-hidden="true" style="color:red;" title="Delete Trade" onclick="fnDeleteThisTrade(' + vLegID + ');"></i></td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.delta, objMet.deltaBase, 2, "grey") + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.gamma, objMet.gammaBase, 4, "grey") + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.theta, objMet.thetaBase, 4, "grey") + '</td>';
        vTempHtml += '<td>' + fnOpsdeGreekCell(objMet.vega, objMet.vegaBase, 4, "grey") + '</td>';
        ...
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.lotSize + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.qty + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.buyPrice.toFixed(2) + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.sellPrice.toFixed(2) + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.charges.toFixed(2) + '</td>';
        vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:grey;">' + objMet.pl.toFixed(2) + '</td>';
    ...
    vTempHtml += '<tr>';
    vTempHtml += '<td></td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalDelta, vTotalDeltaBase, 2, "grey") + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalGamma, vTotalGammaBase, 4, "grey") + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalTheta, vTotalThetaBase, 4, "grey") + '</td>';
    vTempHtml += '<td>' + fnOpsdeGreekCell(vTotalVega, vTotalVegaBase, 4, "grey") + '</td>';
    vTempHtml += '<td></td><td></td><td></td><td></td><td></td><td></td>';
    vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:red; font-weight:bold;">' + vTotalCharges.toFixed(2) + '</td>';
    vTempHtml += '<td style="text-wrap: nowrap; text-align:right; color:white; font-weight:bold;">' + vNetPL.toFixed(2) + '</td>';
    vTempHtml += '<td></td><td></td><td></td>';
