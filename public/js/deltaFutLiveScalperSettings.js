...
function fnGetSetTraderLoginStatus(){
    let vTraderStatus = localStorage.getItem("lsDeltaLoginValidR");
    ...
    if(objApiKey.value === ""){
        ...
        .then(objResult => {
            if(objResult.status === "success"){
                ...
                document.getElementById("spnBalance").innerText = (parseFloat(objBalances.BalanceUSD)).toFixed(2);
                ...
                localStorage.setItem("lsDeltaLoginValidR", "true");
...
function fnSwapCurrency(){
    ...
    let objDisplayCurr = document.getElementById("spnCurrency");
    let objDisplayBal = document.getElementById("spnBalance");

    if(objDisplayCurr.innerText === "USD"){
        objDisplayCurr.innerText = "INR";
        objDisplayBal.innerText = (parseFloat(objNetLimits.BalanceINR)).toFixed(2);
    }
    else if(objDisplayCurr.innerText === "INR"){
        objDisplayCurr.innerText = "USD";
        objDisplayBal.innerText = (parseFloat(objNetLimits.BalanceUSD)).toFixed(2);
    }
}
...
function fnToggleAutoTrader(){
    ...
    if(bAppStatus){
        if(isLsAutoTrader === null || isLsAutoTrader === "false"){
            ...
            localStorage.setItem("isDeltaAutoTraderR", "true");
        }
        else{
            ...
            localStorage.setItem("isDeltaAutoTraderR", "false");
        }
    }
    else{
...
function fnClearLoginStatus(){
    localStorage.removeItem("lsDeltaLoginValidR");
    localStorage.removeItem("isDeltaAutoTraderR");
