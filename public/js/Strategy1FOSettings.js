...
function fnLoadLoginCred(){
    ...
    let vApiKey = JSON.parse(localStorage.getItem("lsApiKeyDSSDV1"));
    let vApiSecret = JSON.parse(localStorage.getItem("lsApiSecretDSSDV1"));
...
function fnGetSetTraderLoginStatus(){
    let vTraderStatus = localStorage.getItem("lsLoginValidDSSDV1");
    ...
    if(objApiKey.value === ""){
        ...
        .then(objResult => {
            if(objResult.status === "success"){
                ...
                localStorage.setItem("lsApiKeyDSSDV1", JSON.stringify(objApiKey.value));
                localStorage.setItem("lsApiSecretDSSDV1", JSON.stringify(objApiSecret.value));
                ...
                // localStorage.setItem("NetLimitDSSDV1", JSON.stringify(objBalances));
                // console.log(localStorage.getItem("NetLimitDSSDV1"));

                $('#mdlDeltaLogin').modal('hide');
                localStorage.setItem("lsLoginValidDSSDV1", "true");
...
function fnToggleAutoTrader(){
    ...
    if(bAppStatus){
        if(isLsAutoTrader === null || isLsAutoTrader === "false"){
            ...
            localStorage.setItem("isAutoTraderDSSDV1", "true");
        }
        else{
            ...
            localStorage.setItem("isAutoTraderDSSDV1", "false");
        }
    }
    else{
...
function fnClearLoginStatus(){
    localStorage.removeItem("lsLoginValidDSSDV1");
    localStorage.removeItem("isAutoTraderDSSDV1");
